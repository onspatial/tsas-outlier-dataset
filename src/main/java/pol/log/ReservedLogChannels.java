...
public class ReservedLogChannels implements Serializable {
	...
	protected Collection<EventList> eventChangingoutlier;
	...
	private static ConcurrentMap<Level, Setting> init() {
		...
		// Output disease
		// Put into the right whatTest LATER
	...
	protected Object[][] getCandidates() {
		...
		Object[][] scheduleCandidates = {
				...
				{ "EVT15", new LogSchedule(0, "EVT15",
						(Supplier & Serializable) () -> "step\tagentId\t[diseaseStatus,byAgentID,outlierType,outlierDegree,time,location,checkin]",
						textFormatter, 0) },
				{ "EVT15",
						new IterativeEventLogSchedule(0, 1, "EVT15",
								(Supplier<Collection<EventList>> & Serializable) () -> eventChangingoutlier,
								eventFormatter, 1) },
	...
	public void loggingSetup() {
		...
		if (eventChangingoutlier == null) {
			eventChangingoutlier = new ArrayList<>();

			// step agentId [diseaseStatus,outlierType,outlierDegree,time,location,checkin]
			for (Person p : model.getAgents()) {
				EventList eventList = new EventList(p.getAgentId());
				eventList.enableIndividualUpdateTime(false);
				InfectiousModel disease = model.getAgent(p.getAgentId()).getInfectiousModel();
				eventList.add((Supplier & Serializable) () -> disease.getStatus());
				eventList.add((Supplier & Serializable) () -> disease.getInfectedByAgentID());

				eventList.add((Supplier & Serializable) () -> p.getOutlierType());
				eventList.add((Supplier & Serializable) () -> p.getOutlier().getOutlierDegree());

				eventList.add((Supplier & Serializable) () -> disease.getStatusChangeTime());
				eventList.add((Supplier & Serializable) () -> disease.getStatusChangeLocation());
				eventList.add((Supplier & Serializable) () -> disease.getStatusChangeCheckIn());

				eventChangingoutlier.add(eventList);
			}
		}
