...
public class FoodNeed implements Need, java.io.Serializable {
	...
	public void update() {
		...
		//// (if statements below shows the effect of eating characteristics on Hunger
		...
		if (isSatisfied() == false && currentMode != PersonMode.Transport
			...
			if (currentMode == PersonMode.AtHome) {
				...
				if (pub.getHourlyCost() <= agent.getFinancialSafetyNeed().getRemainingFoodBudgetForToday()) {
					agent.getFinancialSafetyNeed().withdrawMoney(pub.getHourlyCost(), ExpenseType.Hunger);
					justAte();
	...
	private void eatAtHome() {
		agent.getFinancialSafetyNeed().withdrawMoney(agent.getModel().params.mealCostAtHome, ExpenseType.Hunger);
		justAte();
	}
	...
	public void changeKeepingFullTimeInMinutes(double ratio) {
		this.keepingFullTimeInMinutes *= ratio;
	}

	public void changeFullnessDecreasePerStep(double ratio) {
		this.fullnessDecreasePerStep *= ratio;
	}

	public void resetOutlierAppetite() {
		keepingFullTimeInMinutes = 180 - appetite * 60;
		fullnessDecreasePerStep = this.agent.getModel().getMinutePerStep()
				* appetite * 0.65;
	}
